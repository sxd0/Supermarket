...
export const [cart, setCart] = useState<CartList[] | null>(null);
export const [cards, setCards] = useState<{ [key: number]: Card }>({});
...
export const quantityCard = async (id: number, quantity: number) => {
  if (quantity > 0) {
    try {
